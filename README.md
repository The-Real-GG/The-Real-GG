
<div align="center">

<!-- Header Banner -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=auto&height=200&section=header&text=Hi%20there,%20I'm%20Guhan%20Gargya%20👋&fontSize=38&fontColor=fff&animation=fadeIn" width="100%" />

<!-- Location & Status -->
<p align="center">
  📍 <strong>Location:</strong> Houston, TX &nbsp;|&nbsp; 
  🚀 <strong>Focus:</strong> Computer Science, Mathematics, AI & Data Science
</p>

<!-- Divider -->
<img src="https://capsule-render.vercel.app/api?type=rect&height=2&color=00BFFF&section=footer" width="100%" />

</div>

<!-- About Section -->
<h2 align="center">🧠 About Me</h2>

```text
🎓 CS & Math Student | USACO Gold Division
  ├── 👑 President: CS Club, CSHS & Mu Alpha Theta / UIL Math Team
  ├── 🔬 Researcher: <a href="[https://www.doi.org/10.70251/HYJR2348.44364379](https://www.doi.org/10.70251/HYJR2348.44364379)">Mathematical Models in Quantifying Personal Productivity</a>
  ├── 🎥 Founder @ TUA Academy (Math Prep YouTube Channel)
  └── 📫 Reach me at: guhan.sgargya@gmail.com

```

* **Adversarial Cloaking Researcher** @ *UT Austin*

* Optimized a pipeline designed to deceive object detectors using texture-modified human meshes.


* Authored advanced technical documentation detailing patch initialization, training flow, and transitions from static 3D models to dynamic poses.


* Analyzed gradient-based augmentations to propose solutions for overcoming rigid pose limitations in the dataset.




* **AI Summer Intern** @ *Activera* (2026)


* Built and deployed the production version of a Net Present Value (NPV) Estimation Tool alongside a dedicated GitHub repository and guide.


* Co-developed the *Business First AI Framework*, AI Adoption Rubric, and Excel-based Desire & Readiness Assessment with automated scoring.


* Tailored assessment frameworks specifically for the Energy sector and presented solutions to executive leadership.




* **Summer AI Intern** @ *ZCubes*

* Developed informative presentations covering deep learning and AI concepts.


* Initiated development on a Node.js JavaScript-based LLM implementation.




* **Founder & Lead Instructor** @ *TUA Academy*

* Created a TMSCA General Math preparation YouTube channel featuring over 15 hours of video content and 8,000+ views.


* Taught 70+ students through in-person lessons, driving average score improvements of 30–50 points.





* 📊 **[NPV Estimation Engine](https://www.google.com/search?q=https://github.com/YOUR_USERNAME)**

* Built a production-ready financial estimation engine for analyzing Net Present Value, complete with automated evaluation metrics.


* **Tech Stack:** `Python` `Excel` `Financial Modeling`



* 🚨 **AI Disaster Relief Management**

* Engineered a machine learning model to analyze social media feeds during natural disasters to predict emergency aid requirements (medical, food, energy).


* **Tech Stack:** `Python` `Machine Learning` `Data Science` `NLP`



* 🚘 **Autonomous Self-Driving Car Navigation**

* Built a custom physical robot car and implemented real-time visual pathfinding algorithms using camera data for collision avoidance and navigation.


* **Tech Stack:** `Python` `Computer Vision` `Robotics`



* 📈 **Productivity Quantification Research**

* Formulated a weighted mathematical model to analyze daily personal productivity variables including accuracy, efficiency, and idle time.


* **Tech Stack:** `Data Analysis` `Mathematical Modeling` `Research`




* 🥇 **USACO Gold Division** competitor


* 🥇 **1st Place** – UIL CS District Championship (2026)


* 🥈 **2nd Place** – UIL CS Regional Championship (2026)


* 🥇 **1st Place** – Taylor CS Competition (2025)


* 🥈 **2nd Place** – UIL Math Districts & UIL Invitational A (2024–2026)


* 🥉 **3rd Place** – Clements CS Competition (2025)


* 🏅 **4th Place** – Seven Lakes CS Competition (2025) & Taylor CS Competition (2024)
